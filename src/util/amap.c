...
size_t ahash_string(char *str);

/** Inserts a new KV-pair (STR, VAL) into the bucket BUCK. */
void abucket_insert(abucket *buck, char **str, int val);

/** Doubles the capacity of MAP, and rehashes its contents so that nothing is lost. */
void aexpand(amap *map);

void amap_put(amap *map, char **key, int value) {
    if (!map) {
    return;
    }
    size_t hash = ahash_string(key[0]) % map->num_buckets;
    abucket buck = map->buckets[hash];
    for (size_t i = 0; i < buck.num_pairs; i += 1) {
    if (!strcmp(key[0], buck.pairs[i].key[0]) && !strcmp(key[1], buck.pairs[i].key[1])) {
    ...
    abucket_insert(map->buckets + hash, key, value);
    map->num_pairs += 1;
    } else {
    aexpand(map);
    amap_put(map, key, value);
    }
}

int amap_get(amap *map, char **key) {
    int tmp;
    return amap_get_extended(map, key, &tmp);
}
...
amap *amap_new() {
    amap *ret = safe_malloc(sizeof(amap));
    ret->num_buckets = INIT_CAPACITY;
    ret->num_pairs = 0;
    ret->buckets = safe_calloc(ret->num_buckets * sizeof(abucket));
    return ret;
}
...
void abucket_insert(abucket *buck, char **str, int val) {
    ...
    buck->pairs = safe_realloc(buck->pairs, new_size * sizeof(apair));
    }
...
void aexpand(amap *map) {
    abucket* old_buckets = map->buckets;
    size_t old_num_buckets = map->num_buckets;
    map->buckets = safe_calloc(map->num_buckets * 2 * sizeof(abucket));
    map->num_buckets *= 2;
    map->num_pairs = 0;
    for (size_t i = 0; i < old_num_buckets; i += 1) {
    abucket *cur_bucket = old_buckets + i;
...
void amap_increment(amap *map, char **key, int amt) {
    ...
    int val = amap_get_extended(map, key, &already_contains);
    if (already_contains) {
    amap_put(map, key, val + amt);
    } else {
    amap_put(map, key, amt);
    }
}
